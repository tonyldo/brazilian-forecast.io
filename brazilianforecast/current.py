...
class BrazilianCurrentWeather():
    ...
    def validate_icon_url(self, weather_code, _isNight_sufix=None):
          ...
          if _isNight_sufix is not None:
             resp = requests.get(self._icon_url % (self._host_number_icon_Url, weather_code+_isNight_sufix))
             if resp.status_code == 200:
                return resp.url
    ...
    def get_current_conditions(self,station_id):
        resp = requests.get(self.get_formated_current_situation_URL(station_id))
        if resp.status_code!=200:
            return None
